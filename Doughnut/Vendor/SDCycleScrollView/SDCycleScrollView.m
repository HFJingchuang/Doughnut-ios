...
- (void)layoutSubviews
{
    ...
    if (self.pageControlAliment == SDCycleScrollViewPageContolAlimentCenter) {
        y = self.mainView.sd_height - size.height - 38 - 5;
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"] || [imagePath hasPrefix:@"https"]) {
            [cell.imageView sd_setImageWithPreviousCachedImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage options:SDWebImageRetryFailed progress:nil completed:nil];
